...
#worksheet_loss = workbook.add_sheet('loss')
...
ddpg=Agent.DDPG_agent_att(state_counters=s_dim, action_counters=a_dim, batch_size=BATCH_SIZE, memory_size=MEMORY_SIZE,)
index_=0
var = 0.5  # the controller of exploration which will decay during training process
t1 = time.time()
...
#    s=train_inout_seq[0:1,:]
    ...
    for j in range(EP_STEPS):
       ...
       # print(float(a), float(lab),r)
        ...
        # worksheet_loss.write(index_, i, ddpg.loss)
    print('Episode: ', i,'ep_r: ' ,ep_r)
    worksheet_reward.write(i, 0, ep_r)
print('Running time: ', time.time() - t1)
workbook.save('DDPG_att_data/DDPG_predict__5.xls')
...
true_test.to_csv("DDPG_att_data/ddpg__5.csv")
